          ...
          {activeTab === "ai" && (
            <div className="tab-content" style={{ animation: "fadeIn 0.3s" }}>
              <div className="form-group">
                ...
                <select id="provider" className="input" value={provider} disabled={!storageReady} onChange={(e) => handleProviderChange(e.target.value as AIProvider)}>
                  {AI_PROVIDERS.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label htmlFor="apikey">{config.name} Secret Key</label>
                <input id="apikey" type="password" className="input" placeholder={config.keyPlaceholder} value={apiKey} disabled={!storageReady} onChange={(e) => setApiKey(e.target.value)} />
              ...
              <button className="btn" disabled={!storageReady} onClick={saveKeys}>
